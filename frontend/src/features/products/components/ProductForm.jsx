...
const cellInputCls =
  'w-20 border border-line bg-panel px-2 py-1 text-paper outline-none focus:border-accent';
...
// Images are either new uploads ({ file, preview }) or saved ({ url, fileId }).
const imageSrc = (image) => image.preview || image.url;
const newFiles = (images) => images.filter((image) => image.file);
const savedImages = (images) =>
  images
    .filter((image) => !image.file)
    .map(({ url, fileId, alt }) => ({ url, fileId, alt }));
...
const toFormState = (product) => ({
  ...
  collection: product?.collectionName ?? 'SS24 LUNACORE',
  vendor: product?.vendor ?? 'STITCH FACTORY-01',
  sku: product?.sku || '',
});

/**
 * Create / edit form shared by the admin product pages.
 * `onSubmit(formData)` must resolve to `{ success, error? }`.
 */
const ProductForm = ({ product, onSubmit, saving, error }) => {
  ...
  // Every field is sent so edits can also clear values.
  const buildFormData = (status) => {
    ...
    // Variant metadata travels as JSON; each variant's new files are sent as
    // `variantImages_<index>` so the API can pair them up by position.
    formData.append(
      'variants',
      JSON.stringify(
        variants.map((v) => ({
          ...
          images: savedImages(v.images || []),
        })),
      ),
    );
    variants.forEach((v, index) => {
      newFiles(v.images || []).forEach(({ file }) => {
        formData.append(`variantImages_${index}`, file);
      });
    });

    const uniqueColorways = new Map();
    variants.forEach((v) => {
      if (v.colorway?.name) uniqueColorways.set(v.colorway.name, v.colorway);
    });
    formData.append(
      'colorways',
      JSON.stringify(Array.from(uniqueColorways.values())),
    );

    if (isEdit) {
      formData.append('existingImages', JSON.stringify(savedImages(images)));
    }
    newFiles(images).forEach(({ file }) => formData.append('images', file));
    return formData;
  };

  const submit = async (status) => {
    const result = await onSubmit(buildFormData(status));
  ...
  const saveAsDraft = (e) => {
    // Runs the browser's required-field checks before saving.
    if (e.currentTarget.form.reportValidity()) submit('draft');
  };

  const previewColorways = [
    ...new Map(
      variants
    ...
    <form onSubmit={handleSubmit}>
      ...
      <header className="sticky top-0 z-40 flex h-16 items-center justify-between border-b border-line bg-ink/80 px-6 backdrop-blur-md">
        <div className="flex items-center gap-2 font-display text-[11px] font-bold uppercase tracking-[0.12em]">
          ...
          <span className="text-paper">{isEdit ? 'Edit' : 'New'}</span>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            onClick={saveAsDraft}
            disabled={saving}
            className="px-2 py-2 font-display text-[11px] font-bold uppercase tracking-[0.12em] text-muted transition-colors hover:text-paper disabled:opacity-60"
          >
            Save as Draft
          </button>
          <button
      ...
      <div className="mx-auto w-full max-w-[1440px] p-6">
        <div className="mb-8">
          ...
          <p className="max-w-2xl text-sm text-muted">
            {isEdit
              ? 'Update details, media, stock and variants. Removed images are deleted from storage when you save.'
              : 'Configure a new SKU. Ensure technical specifications and variant details are precise.'}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-12">
          ...
          <div className="space-y-6 lg:col-span-8">
            ...
            <section className={cardCls}>
              ...
              <div className="grid grid-cols-4 gap-4">
                <div
                  ...
                  onDrop={handleDrop}
                  className={`group col-span-4 flex h-60 cursor-pointer flex-col items-center justify-center border-2 border-dashed transition-colors ${
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      title={
                        variants.length > 0
                          ? 'Stock is the total of the variants below'
                          : undefined
                      }
            ...
            <section className={cardCls}>
              ...
              <div className="mb-6 space-y-4 border border-line/80 bg-panel p-4">
              ...
              {variants.length > 0 ? (
                <div className="overflow-x-auto border border-line">
                  <table className="w-full text-left font-display text-[11px]">
                    <thead className="border-b border-line bg-panel uppercase tracking-wider text-muted">
                      <tr>
                    ...
                    <tbody className="divide-y divide-line/60 bg-field">
                      {variants.map((v, index) => (
                        ...
                        >
                          ...
                          <td className="p-3 tracking-wide text-muted">
                            {v.sku || 'AUTO-GEN'}
                          </td>
                          <td className="p-3">
                            <input
                              type="number"
                              min="0"
                              aria-label={`Stock for ${v.size} ${v.colorway?.name}`}
                              value={v.stock}
                              onChange={(e) =>
                                updateVariant(index, {
                          ...
                          <td className="p-3">
                            <input
                              ...
                              min="0"
                              step="0.01"
                              aria-label={`Price for ${v.size} ${v.colorway?.name}`}
          ...
          <div className="space-y-6 lg:sticky lg:top-[88px] lg:col-span-4">
            ...
            <section className={cardCls}>
              ...
              <p className="text-[10px] uppercase leading-relaxed tracking-wide text-faint">
                Visibility: this product is hidden from the storefront unless it
                is active.
              </p>
            </section>
            ...
            <section className={cardCls}>
              ...
              <div className="border border-line bg-ink p-4">
                <div className="relative mb-4 aspect-[3/4] overflow-hidden bg-panel">
                  ...
                  {form.collection && (
                    <span className="absolute right-2 top-2 bg-accent px-1.5 py-0.5 font-display text-[8px] font-bold uppercase tracking-[0.2em] text-ink">
                      {form.collection}
                    </span>
                  )}
                </div>
                <div className="space-y-1">
                  <p className="font-display text-[9px] font-bold uppercase tracking-[0.2em] text-muted">
                ...
                {previewColorways.length > 0 && (
                  <div className="mt-4 flex gap-1.5">
                    {previewColorways.map((colorway) => (
                      <span
                        key={colorway.name}
                        title={colorway.name}
              ...
              {isEdit && product.status === 'active' && (
                <p className="mt-4 text-center">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-1 text-[10px] uppercase tracking-wide text-muted underline underline-offset-4 transition-colors hover:text-accent"
                  >
